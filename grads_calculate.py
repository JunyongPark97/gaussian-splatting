...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    global selected_random_pts_v, query_filter_data
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        print('VISIBIL POINTS...! SIZE -> ', visible_points.size())

        # TODO : random select by percent
        # 랜덤하게 뽑은 데이터 5%
        print("ITERATION :::", iteration)
        if ii == 0:
            selected_random_pts_v = select_random_pts(visible_points, percent)  # 랜덤 선택된 포인트 좌표 리스트
            # TODO : make selected mask -> moving points
            # 랜덤하게 뽑은 데이터의 True False Indicator <=> gaussian xyz 와 같은 차원(25만개) 이걸로 move할 수 있음
            query_filter_data = random_query_filter(gaussians.get_xyz, random_v)
            # query_filter_data = query_filter(gaussians.get_xyz, selected_random_pts_v)
            print('QFD ::::', query_filter_data.size())
        ...
        with torch.no_grad():
            ...
            # 5번째일때 저장한 값들 불러와서 sort하고 mask 추출해서 position 업데이트
            if ii == 5:

                #TODO : xyz 지우고 저장해서 보기, 지우는 조건 2개; 0 and threshold -> 지워진 애들만 저장 , 각각 지우고 저장
                th_mask = make_th_mask(save_path)
                zero_mask = make_zero_mask(save_path)
                print("########SAVING#########")
...
if __name__ == "__main__":
    ...
    op = OptimizationParams(parser)  ## 여기서 arguments.py __init__ 값 불러옴
